...
// import i18nInstance from './validateUrl';
import axios, { AxiosError } from 'axios';
// import * as Yup from 'yup';
// import resources from './locales/index';
// import parser from './parser';
import builtUpdate from './builtUpdate';
import createContainerPost from './createContainerPost';
...
// validateUrl(url, state.validLinks, i18nInstance)
// .then(() => {
...
const render = (value, state, i18n) => {
  ...
  if (value === 'filling') {
  //   validateUrl(state.link, state.validLinks)
  //     .then(() => {
  ...
  //       createContainerPost(state.content, i18nInstance, value);
  //       inputText.value = '';
  //       inputText.focus();
  ...
  //         .then((response) => {
  //           const data = parser(response);
                const difference = builtUpdate(state.link, state.content);
